...
import { Plus, Folder, Copy, Check, Shield, Layers, X } from 'lucide-react';
...
export const Sidebar: React.FC<Props> = ({
  ...
  const handleCopyInvite = () => {
    if (activeWorkspace?.inviteCode) {
      ...
      setCopied(true);
      if (onCopyFeedback) {
        onCopyFeedback(`Invite code ${activeWorkspace.inviteCode} copied to clipboard!`);
      }
  ...
  const handleSelectWorkspaceWrapper = (ws: Workspace) => {
    onSelectWorkspace(ws);
    if (onCloseMobile) onCloseMobile();
  };

  const handleSelectProjectWrapper = (id: string) => {
    onSelectProject(id);
    if (onCloseMobile) onCloseMobile();
  };
      ...
      <aside className={`sidebar ${isMobileOpen ? 'sidebar-mobile-open' : ''}`}>
        ...
        <div style={{ padding: 16, borderBottom: '1px solid #334155' }}>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              if (selected) handleSelectWorkspaceWrapper(selected);
            }}
          >
            {workspaces.map((ws) => (
        ...
        <div style={{ flex: 1, padding: 16, overflowY: 'auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <button
              onClick={() => handleSelectProjectWrapper('')}
              style={{
                display: 'flex',
                alignItems: 'center',
                gap: 8,
            ...
            {projects.map((proj) => (
              <button
                key={proj._id}
                onClick={() => handleSelectProjectWrapper(proj._id)}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  gap: 8,
